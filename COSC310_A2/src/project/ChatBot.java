...
/*
 * contains LanguageToolkit version 1.1 implementation for evaluating user responses
 */
...
public class ChatBot {
	...
	/**
	 * variables that store some possible inputs by the user, which we use to
	 * compare in methods below to see if the responses are positive or negative.
	 * Also have greetings and goodbyes (set dialogues for the conversation)
	 */
	private static String[] positiveInput = { "yes", "yeah", "y", "yep", "affirmative", "positive", "agreed", "sure",
			"ok", "fine" };
	private static String[] negativeInput = { "no", "nah", "n", "nop", "negative", "disagree", "not", "disagreed",
			"never", "nope" };
	...
	private static String[] negativeResponse = {
			"Im sorry you are upset, but the only thing I can do a recommendation to watch movies or tv shows, would you like one?",
	...
	private static String[] Greeting = { "Hey would you like to know what I can do?",
			"Do you wish to explore my functionality?", "Whats up, do want to know something?" };
	...
	public static void main(String[] args) {
		...
		while (count >= 0) {
			...
			if (count == 0) {
				...
				input = scan.nextLine();
				input.toLowerCase();
				// check if the user had a possitve response and proceed with the narrative and
				// checks the inputs validity
				...
				if (checkString(input) == true) {
					...
					if (checkResponse(input, count) == true) {
						...
						// if the second response (which asked if they were sure they wanted to not
						// converse)
						// open the webscrapper object and give them output with movies recommendations,
						// etc
	...
	// checks the state of the resonses and sets the variables accordingly to fit
	// the agents narrative
	public static boolean checkResponse(String str, int count) throws IOException {
	...
	// For simplicity's sake this evaluates the sentiment of the user input
	...
	// this method checks if the user repsonse is negative by interating through the
	// list of negative responses declared in the class above
	public static boolean negativeResponse(String str) throws IOException {
		String posModel_filePath = "filepath";
		// Create language toolkit object with desired model
		LanguageToolkit tool = new LanguageToolkit(posModel_filePath);
		// Determine coref and
	...
	// This method will convert str into a complete lowercase String to standarize
	// all user input to lowercase and
	// check if there is any non alphabetical character inside the string in order
	// to ask the user to retype its input
	public static boolean checkString(String str) {
		char ch[] = str.toCharArray();
		for (int i = 0; i < str.length(); i++) {
			if ((Character.isAlphabetic(ch[i]) == false && str.length() > 1) || ch[i] == ' '
					|| spelling(str, positiveInput, negativeInput) == false) {
				System.out.println("There is an invalid character in your input " + ch[i]
						+ " please reenter your input with valid form or potential spelling mistake");
				return false;
			}
		}
	...
	// this method checks to see if the stemmed inputted response from the user
	// matches any of the positive and negative repsonses
	// basically eliminates any spelling mistakes to a confirmation or a negation
	// (yes and no type response)
